...
sleep(1)
agi.stream_file("/root/emotiphone/asterisk/sounds/welcome") # welcome to the emotiphone recording system
sleep(1)
...
while looping
	...
	# agi.background("/root/emotiphone/asterisk/sounds/begin_recording") # "press 1 to begin a recording, and any key to end"

	result = agi.wait_for_digits("/root/emotiphone/asterisk/sounds/begin_recording", -1, 1) # wait forever
	...
	if result.digits
		...
		if result.digits == '1'
			...
			record_file = "/root/emotiphone/asterisk/recordings/" + relative_path
			agi.record_file(record_file, "WAV", "0123456789#*", 10000, true)

			# agi.stream_file('/root/emotiphone/asterisk/sounds/save_or_discard')

			# confirm = agi.wait_for_digit(-1)
			confirming = true

			while confirming
				cmd = "sox #{record_file}.WAV /root/emotiphone/asterisk/recordings/tmp.WAV silence 1 0.1 1%"
				move = "mv /root/emotiphone/asterisk/recordings/tmp.WAV #{record_file}.WAV"
				system( cmd )
				system( move )					
				
				confirm = agi.wait_for_digits("/root/emotiphone/asterisk/sounds/save_playback_discard", -1, 1) # wait forever
				...
				if confirm.digits == '1'
					...
					agi.stream_file('/root/emotiphone/asterisk/sounds/recording_saved')
					...
					agi.stream_file('/root/emotiphone/asterisk/sounds/recording_discarded')
				end
			end
		end
